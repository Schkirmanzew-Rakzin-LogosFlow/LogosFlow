...
- **Traefik entrypoint for services**: [http://localhost:8081](http://localhost:8081) (routes `/litellm`, `/mcpo`, `/mcpo/crawl4ai`)
- **LiteLLM API (direct)**: [http://localhost:4000](http://localhost:4000)
- **Ollama API (direct)**: [http://localhost:11434](http://localhost:11434)
- **Keycloak Admin Console**: [http://localhost:49088/admin/](http://localhost:49088/admin/)

### LLM Stack Notes

- Components: `ollama_server` (GPU enabled), `model_prep` (registers models), `litellm_proxy` (OpenAI-compatible proxy), `mcpo`, and `crawl4ai` (exposed via Traefik at `/mcpo` and `/mcpo/crawl4ai` on port 8081).
- Desired models are declared in `docker/llm/models.list` (one GGUF filename per line). At startup `docker/llm/scripts/init-models.sh` registers only those files from `docker/llm/modelslinks`, deriving tags like `:ctx4096`/`:ctx8192` and attaching projectors for vision models.
- With `PRUNE_ORPHANS=1` (default) the init script prunes any managed `:ctx*` Ollama models not listed in `docker/llm/models.list`; set it to `0` to disable pruning.
- Current GGUF files create the following tags used by LiteLLM: `qwen3-4b-q4_k_m:ctx4096/ctx8192`, `granite-4.0-h-tiny-q4_k_m:ctx4096/ctx8192`, `qwen3-vl-4b-instruct-q4_k_m:ctx4096/ctx8192`. Vision models automatically attach their paired projector files (e.g., `mmproj-Qwen3-VL-4B-Instruct-F16.gguf`) so image inputs work out of the box.
- Ollama is started with `OLLAMA_LLAMACPP_OPTS=--gpu-layers 999` to fully offload supported models to the GPU.
- OpenWebUI points at the LiteLLM proxy (`OPENAI_API_BASE_URL=http://litellm_proxy:4000`) and uses the dummy key `sk-logosflow-local`.
